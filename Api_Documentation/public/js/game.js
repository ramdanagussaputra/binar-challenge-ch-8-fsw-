...
class SaveScore {
    constructor() {
        btnSaveScore.addEventListener('click', this._postScore.bind(this));
    }
    async _postScore() {
        const res = await fetch('http://localhost:7000/api/user-game-history', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify(this._scoreData()),
        });
    ...
    _scoreData() {
        const [win, lose, draw] = result.score;
